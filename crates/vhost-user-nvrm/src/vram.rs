...
/// is the host card's name, in the format `LEA_GPU_NAME_FORMAT` selects (legacy by
/// default, so the examples above are unchanged).
...
/// `LEA_GPU_NAME_FORMAT`, read once. An unknown value keeps the legacy name, loudly:
...
fn name_format() -> naming::NameFormat {
    ...
    *FORMAT.get_or_init(|| {
        naming::NameFormat::from_env().unwrap_or_else(|e| {
            eprintln!("vhost-user-nvrm: {e}; keeping the legacy name format");
            naming::NameFormat::Legacy
        })
    })
}
...
mod tests {
    ...
    /// The transport word is opt-in; the default stays the string the v1 gates compare.
    #[test]
    fn the_transport_word_is_opt_in() {
        let real = "NVIDIA GeForce RTX 2070";
        ...
        assert_eq!(
            guest_card_name_as::<nvrm_sys::DefaultAbi>(real, p, naming::NameFormat::Transport),
