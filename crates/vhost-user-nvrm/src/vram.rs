...
/// Replace vendor prefixes with Leandro and the transport word, and append the guest
/// framebuffer size. Examples: Leandro VirtIO RTX 2070, Leandro VirtIO RTX 2070-3G,
/// Leandro VirtIO RTX 2070-1536M (Leandro RTX 2070-3G with LEA_GPU_NAME_FORMAT=legacy).
/// Drop a suffix that would exceed the name buffer; never truncate its size.
/// The name is generated by [`nvrm_abi::naming`] from a VirtIO spec whose personality
/// is the host card's name, in the format `LEA_GPU_NAME_FORMAT` selects (transport by
/// default since 2026-09-24).
pub fn guest_card_name<A: RmAbi>(real: &str, profile: Profile) -> String {
    guest_card_name_as::<A>(real, profile, name_format())
}

/// [`guest_card_name`] in an explicit format.
pub fn guest_card_name_as<A: RmAbi>(
    real: &str,
    profile: Profile,
    format: naming::NameFormat,
) -> String {
    naming::VirtualGpuSpec {
        transport: naming::Transport::Virtio,
        personality: naming::Personality::from_driver_name(real),
        profile: naming::Profile::from_fb_bytes(profile.fb_length),
        backend: naming::Backend::OpenRm,
    }
    .guest_name_within(format, name_max::<nvrm_sys::DefaultAbi>())
}

/// `LEA_GPU_NAME_FORMAT`, read once. An unknown value keeps the default name, loudly:
/// the name is cosmetic, and refusing to start over it would take the VM down.
fn name_format() -> naming::NameFormat {
    static FORMAT: std::sync::OnceLock<naming::NameFormat> = std::sync::OnceLock::new();
    *FORMAT.get_or_init(|| {
        naming::NameFormat::from_env().unwrap_or_else(|e| {
            eprintln!("vhost-user-nvrm: {e}; keeping the default name format");
            naming::NameFormat::default()
        })
    })
}
...
mod tests {
    ...
    fn the_card_says_what_it_is() {
        ...
        assert_eq!(
            ...
            "Leandro VirtIO A100-SXM4-40GB-10G"
        );
    }

    /// The transport word is the default (2026-09-24); the legacy name, the string
    /// the v1 gates compared before, stays available as an explicit opt-in.
    #[test]
    fn the_transport_word_is_the_default_and_legacy_stays() {
        let real = "NVIDIA GeForce RTX 2070";
        let p = Profile::accounting(4096 << 20);
        assert_eq!(
            guest_card_name_as::<nvrm_sys::DefaultAbi>(real, p, naming::NameFormat::default()),
            "Leandro VirtIO RTX 2070-4G"
        );
        assert_eq!(
            guest_card_name_as::<nvrm_sys::DefaultAbi>(real, p, naming::NameFormat::Legacy),
            "Leandro RTX 2070-4G"
        );
        assert_eq!(
            guest_card_name_as::<nvrm_sys::DefaultAbi>(
                real,
                Profile::accounting(1333 << 20),
                naming::NameFormat::Transport
            ),
            "Leandro VirtIO RTX 2070-1333M"
        );
    }
    ...
    fn a_name_that_does_not_fit_loses_the_suffix_whole() {
        use naming::NameFormat::Legacy;
        // Legacy, 8 + 53 + 3 = 64 -> does not fit, so the suffix goes as a unit
        ...
        // The default format, `Leandro VirtIO ` (15 bytes): the same three cases.
        let b46 = "X".repeat(46);
        assert_eq!(
            guest_card_name::<nvrm_sys::DefaultAbi>(
                &format!("NVIDIA GeForce {b46}"),
                Profile::accounting(2048 << 20)
            ),
            format!("Leandro VirtIO {b46}")
        );
        let b49 = "X".repeat(49);
        assert_eq!(
            guest_card_name::<nvrm_sys::DefaultAbi>(
                &format!("NVIDIA GeForce {b49}"),
                Profile::accounting(2048 << 20)
            ),
            "Leandro GPU"
        );
        let b48 = "X".repeat(48);
        assert_eq!(
            guest_card_name::<nvrm_sys::DefaultAbi>(&format!("NVIDIA GeForce {b48}"), Profile::OFF)
                .len(),
            63
        );
    }
    ...
    fn the_name_is_written_nul_terminated() {
        ...
        assert_eq!(
            rewrite_gpu_name::<nvrm_sys::DefaultAbi>(&mut v, Profile::accounting(2048 << 20))
                .as_deref(),
            Some("Leandro VirtIO RTX 2070-2G")
        );
