...
int main(void)
{
	...
	if (e != CL_SUCCESS || nplat == 0) {
		...
		fprintf(stderr,
			"oclprobe: no OpenCL platform (loader found no vendor library)\n");
		return 1;
	}
	...
	for (cl_uint i = 0; i < nplat; i++) {
		char v[256] = { 0 };
		if (clGetPlatformInfo(plats[i], CL_PLATFORM_VENDOR, sizeof v, v,
				      NULL) != CL_SUCCESS)
			continue;
		if (strstr(v, "NVIDIA")) {
			plat = plats[i];
	...
	if (!plat) {
		fprintf(stderr,
			"oclprobe: %u platform(s), none of them NVIDIA\n",
			nplat);
		return 1;
	}

	cl_device_id dev;
	if ((e = clGetDeviceIDs(plat, CL_DEVICE_TYPE_GPU, 1, &dev, NULL)) !=
	    CL_SUCCESS)
	...
	// the CUDA JIT.
	if ((e = clBuildProgram(prog, 1, &dev, NULL, NULL, NULL)) !=
	    CL_SUCCESS) {
		char log[4096] = { 0 };
	...
	cl_mem dc = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, N * sizeof(int),
				   NULL, &e);
	if (!dc)
	...
	size_t global = N;
	if ((e = clEnqueueNDRangeKernel(q, k, 1, NULL, &global, NULL, 0, NULL,
					NULL)) != CL_SUCCESS)
		return fail("clEnqueueNDRangeKernel", e);
	if ((e = clEnqueueReadBuffer(q, dc, CL_TRUE, 0, N * sizeof(int), c, 0,
