...
static uint32_t alloc_client(int fd, int *ok)
{
	...
	*ok = (r == 0 && p.status == 0);
	if (!*ok)
		fprintf(stderr, "  alloc client: ret=%d status=0x%x\n", r,
			p.status);
	return p.hObjectNew;
}
...
int main(void)
{
	...
	probe("erfundener Client-Handle (Kontrolle)", fd1, 0xc1d0dead,
	      0xc1d0dead);
	...
	if (pid == 0) {
		...
		probe("PARENT's client on the INHERITED OFD (fd1)", fd1, c1,
		      c1);
