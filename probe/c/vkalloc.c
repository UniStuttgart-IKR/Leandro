...
int main(int argc, char **argv)
{
	...
	for (uint32_t i = 0; i < n; i++) {
		...
		printf("    [%u] %s\n", i, props.deviceName);
		if (!pick && strstr(props.deviceName, want) &&
		    !strstr(props.deviceName, "Venus"))
			pick = devs[i];
	}
	if (!pick) {
		printf("  no device matching '%s'\n", want);
		return 1;
	}
	...
	uint32_t nde = 0;
	if (have_fd)
		devext[nde++] = VK_KHR_EXTERNAL_MEMORY_FD_EXTENSION_NAME;
	if (have_dmabuf)
		devext[nde++] = VK_EXT_EXTERNAL_MEMORY_DMA_BUF_EXTENSION_NAME;
	if (have_hostptr)
		devext[nde++] = VK_EXT_EXTERNAL_MEMORY_HOST_EXTENSION_NAME;
	...
	for (uint32_t i = 0; i < mp.memoryTypeCount; i++) {
		...
		if (a == VK_SUCCESS) {
			if (mp.memoryTypes[i].propertyFlags &
				...
				void *p = NULL;
				VkResult m = vkMapMemory(dev, mem, 0,
							 VK_WHOLE_SIZE, 0, &p);
				mapres = res(m);
				if (m == VK_SUCCESS)
		...
		printf("  %-4u %-6u %-46s %-10s %s\n", i,
		       mp.memoryTypes[i].heapIndex, f, res(a), mapres);
	}
	...
	if (r == VK_SUCCESS) {
		...
		VkMemoryAllocateInfo ai = {
			...
			.memoryTypeIndex =
				(uint32_t)__builtin_ctz(mr.memoryTypeBits)
		};
		VkDeviceMemory mem;
		VkResult a = vkAllocateMemory(dev, &ai, NULL, &mem);
		printf("  %-40s %s (type %u of mask %#x)\n", "  bind memory",
		       res(a), (uint32_t)__builtin_ctz(mr.memoryTypeBits),
		       mr.memoryTypeBits);
		if (a == VK_SUCCESS) {
			printf("  %-40s %s\n", "  vkBindBufferMemory",
	...
	if (r == VK_SUCCESS) {
		...
		printf("  %-40s %s (%llu bytes, type %u of mask %#x)\n",
		       "  bind memory", res(a), (unsigned long long)mr.size,
		       (uint32_t)__builtin_ctz(mr.memoryTypeBits),
		       mr.memoryTypeBits);
		if (a == VK_SUCCESS) {
			printf("  %-40s %s\n", "  vkBindImageMemory",
	...
	for (int pass = 0; pass < 2; pass++) {
		...
		VkResult q2 = vkGetPhysicalDeviceImageFormatProperties2(
			pick, &ifi, &ifp);
		...
		printf("    %-38s %s (%llu bytes, type %u heap %u: %s)\n",
		       ...
		       (unsigned long long)mr.size, xt,
		       mp.memoryTypes[xt].heapIndex, pf);
		if (xa == VK_SUCCESS) {
			printf("    %-38s %s\n", "vkBindImageMemory",
			...
			if (!getfd) {
				...
				if (g == VK_SUCCESS && fd >= 0) {
					snprintf(lp, sizeof lp,
						 "/proc/self/fd/%d", fd);
					ssize_t rl = readlink(lp, tgt,
							      sizeof tgt - 1);
					if (rl > 0)
						tgt[rl] = 0;
				}
				printf("    %-38s %s  fd=%d (%s)\n",
				       "vkGetMemoryFdKHR", res(g), fd, tgt);
				if (fd >= 0)
					close(fd);
			}
	...
	if (have_hostptr) {
		...
		if (posix_memalign(&host, align, len) != 0 || !host) {
			...
			printf("  %-40s %s (types %#x)\n",
			       "vkGetMemoryHostPointerProperties", res(hr),
			       hpp.memoryTypeBits);

			if (hr == VK_SUCCESS && hpp.memoryTypeBits) {
				uint32_t ht2 = (uint32_t)__builtin_ctz(
					hpp.memoryTypeBits);
				char pf2[128];
				flags_str(mp.memoryTypes[ht2].propertyFlags,
					  pf2, sizeof pf2);
				...
				VkDeviceMemory imem;
				VkResult ia = vkAllocateMemory(dev, &iai, NULL,
							       &imem);
				printf("  %-40s %s (type %u heap %u: %s)\n",
				       "vkAllocateMemory (imported host ptr)",
				       res(ia), ht2,
				       mp.memoryTypes[ht2].heapIndex, pf2);
				if (ia == VK_SUCCESS) {
