...
int main(int argc, char **argv)
{
	...
	ck(cuModuleGetFunction(&fadd, mod, "vecAdd"),
	...
	{
		CUfunction fspin;
		ck(cuModuleGetFunction(&fspin, mod, "spin"),
		   "cuModuleGetFunction spin");

		const char *e;
		long long cycles = (e = getenv("NVPROBE_CYCLES")) ? atoll(e) :
								    30000000LL;
		 ...
		 * the average. */
		ck(cuLaunchKernel(fspin, 1, 1, 1, 32, 1, 1, 0, 0, sargs, NULL),
		   "spin warmup");
		...
		for (int i = 0; i < iters; i++) {
			ck(cuLaunchKernel(fspin, 1, 1, 1, 32, 1, 1, 0, 0, sargs,
					  NULL),
			   "cuLaunchKernel spin");
		...
		double ms = ((double)(t1.tv_sec - t0.tv_sec) * 1e3 +
			    ...
			    iters;
		fprintf(stderr,
			"stage 4 ok (%d launches, %.3f ms/launch, sched=%s)\n",
			iters, ms, schedname);
