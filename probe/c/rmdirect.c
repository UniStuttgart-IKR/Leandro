...
struct probed_ids {
	uint32_t gpuIds[MAX_GPUS], excluded[MAX_GPUS], flags[MAX_GPUS];
};
struct attach_ids {
	uint32_t gpuIds[MAX_GPUS], failedId;
};
struct detach_ids {
	uint32_t gpuIds[MAX_GPUS];
};
...
static int ctrl(uint32_t hclient, uint32_t hobject, uint32_t cmd, void *params,
		uint32_t len, const char *what)
{
	...
	int r = ioctl(fd,
		      _IOWR(NV_IOCTL_MAGIC, NV_ESC_RM_CONTROL, struct nvos54),
		      &c);
	if (r != 0 || c.status != 0) {
...
static int open_mig_monitor_cap(const char **why)
{
	...
	fclose(f);
	if (minor < 0) {
		*why = MIG_MONITOR_PROC " has no DeviceFileMinor";
		return -1;
	}
...
static int alloc(uint32_t hroot, uint32_t hparent, uint32_t hnew,
		 uint32_t hclass, void *parms, const char *what)
{
	struct nvos64 a;
	memset(&a, 0, sizeof a);
	a.hRoot = hroot;
	...
	int r = ioctl(fd, _IOWR(NV_IOCTL_MAGIC, NV_ESC_RM_ALLOC, struct nvos64),
		      &a);
	if (r != 0 || a.status != 0) {
...
int main(void)
{
	...
	if (ioctl(fd, _IOWR(NV_IOCTL_MAGIC, NV_ESC_RM_ALLOC, struct nvos64),
		  &root) != 0 ||
	    root.status != 0) {
	...
	int have_probed = ctrl(hc, hc, CMD_GPU_GET_PROBED_IDS, &probed,
			       (uint32_t)sizeof probed,
			       "GPU_GET_PROBED_IDS") == 0;
	...
	if (n_ids > 0) {
		...
		at.failedId = 0;
		attached = ctrl(hc, hc, CMD_GPU_ATTACH_IDS, &at,
				(uint32_t)sizeof at, "GPU_ATTACH_IDS") == 0;
	} else {
	...
	dp.deviceId = 0;
	int have_dev = alloc(hc, hc, H_DEVICE, NV01_DEVICE_0, &dp, "device") ==
		       0;

	if (have_dev) {
		uint32_t sub = 0; /* NV2080_ALLOC_PARAMETERS { subDeviceId } */
		if (alloc(hc, H_DEVICE, H_SUBDEVICE, NV20_SUBDEVICE_0, &sub,
			  "subdevice") == 0) {
			uint64_t t = 0;
			ctrl(hc, H_SUBDEVICE, CMD_TIMER_GET_TIME, &t,
			     (uint32_t)sizeof t, "TIMER_GET_TIME");
		...
		for (int k = 0; k < 2; k++) {
			...
			if (ctrl(hc, H_DEVICE, CMD_GR_GET_CAPS_V2, caps,
		...
		if (alloc(hc, H_DEVICE, H_DISPLAY, NV04_DISPLAY_COMMON, NULL,
			  "display") == 0) {
			uint8_t caps[2] = { 0, 0 };
			ctrl(hc, H_DISPLAY, CMD_SYSTEM_GET_CAPS_V2, caps,
			     (uint32_t)sizeof caps, "SYSTEM_GET_CAPS_V2");
	 ...
	 */
	{
		...
		int capfd = getenv("LEA_RMDIRECT_NO_CAP") ?
				    -1 :
				    open_mig_monitor_cap(&why);
		if (getenv("LEA_RMDIRECT_NO_CAP"))
			why = "(LEA_RMDIRECT_NO_CAP=1 -- not opened)",
			errno = 0;
		printf("  smc capability: %s -> %s\n", why,
		       capfd >= 0 ? "open" :
				    (errno ? strerror(errno) : "skipped"));
		...
		uint64_t capDescriptor =
			(capfd >= 0) ? (uint64_t)(uint32_t)capfd : (uint64_t)-1;
		...
		printf("  alloc %-16s class=%#06x ret=%d status=%#x handle=%#x\n",
		       "smc monitor", AMPERE_SMC_MONITOR_SESSION, r, a.status,
		       a.hObjectNew);
	...
	if (attached) {
		...
		ctrl(hc, hc, CMD_GPU_DETACH_IDS, &dt, (uint32_t)sizeof dt,
		     "GPU_DETACH_IDS");
	}
