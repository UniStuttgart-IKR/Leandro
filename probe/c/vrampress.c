...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "--fill") && i + 1 < argc)
			fill = atol(argv[++i]);
			...
			chunk_mib = atol(argv[++i]);
		else if (!strcmp(argv[i], "--churn") && i + 1 < argc)
			churn_mib = atol(argv[++i]);
		else {
	...
	if (fill >= 0 && grow_max) {
		fprintf(stderr,
			"vrampress: --fill and --max are two different loads; pick one\n");
		return 2;
	}
	...
	printf("vrampress: start free %zu MiB of %zu MiB, mode %s, chunk %ld MiB, churn %ld MiB, %ld s\n",
	       freeb >> 20, totalb >> 20, grow_max ? "max" : "fill", chunk_mib,
	       churn_mib, seconds);
	fflush(stdout);
	...
	size_t target = grow_max ?
				(size_t)-1 :
				(size_t)((double)freeb * (double)fill / 100.0);
	size_t held_bytes = 0;
	double t0 = now();
	...
	while (now() - t0 < (double)seconds) {
		...
		if (r == CUDA_SUCCESS) {
			...
			if (churn_n == 8) {
				...
				memmove(&churn_p[0], &churn_p[1],
					7 * sizeof churn_p[0]);
				memmove(&churn_b[0], &churn_b[1],
					7 * sizeof churn_b[0]);
				churn_n = 7;
			}
		...
		if (now() >= next) {
			...
			printf("%.0f,%zu,%zu,%zu,%lu,%lu,%lu\n", now() - t0,
			       (held_bytes + churn_held) >> 20, freeb >> 20,
			       totalb >> 20, allocs, frees, fails);
			fflush(stdout);
			next = now() + 1.0;
		}
		usleep(20000);
