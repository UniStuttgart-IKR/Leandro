...
int main(void)
{
	...
	cuDeviceGetAttribute(&concurrent,
			     CU_DEVICE_ATTRIBUTE_CONCURRENT_MANAGED_ACCESS,
			     dev);
	cuDeviceGetAttribute(&pageable,
	...
	ck(cuMemAllocManaged(&ma, MN * sizeof(float), CU_MEM_ATTACH_GLOBAL),
	   "AllocManaged a");
	ck(cuMemAllocManaged(&mb, MN * sizeof(float), CU_MEM_ATTACH_GLOBAL),
	   "AllocManaged b");
	ck(cuMemAllocManaged(&mc, MN * sizeof(float), CU_MEM_ATTACH_GLOBAL),
	   "AllocManaged c");
	...
	ck(cuLaunchKernel(fadd, (MN + 255) / 256, 1, 1, 256, 1, 1, 0, 0, args,
			  NULL),
	   "cuLaunchKernel");
	...
	CUmemLocation locd = { .type = CU_MEM_LOCATION_TYPE_DEVICE,
			       .id = (int)dev };
	CUmemLocation loch = { .type = CU_MEM_LOCATION_TYPE_HOST, .id = 0 };
	ck(cuMemPrefetchAsync(ma, MN * sizeof(float), locd, 0, 0),
	   "Prefetch->GPU");
	ck(cuMemAdvise(mb, MN * sizeof(float), CU_MEM_ADVISE_SET_READ_MOSTLY,
		       locd),
	   "Advise");
	ck(cuCtxSynchronize(), "sync prefetch");
	ck(cuMemPrefetchAsync(ma, MN * sizeof(float), loch, 0, 0),
	   "Prefetch->CPU");
	...
	ck(cuMemAdvise(mb, MN * sizeof(float), CU_MEM_ADVISE_SET_READ_MOSTLY,
	   ...
	   "Advise");
	ck(cuCtxSynchronize(), "sync prefetch");
	ck(cuMemPrefetchAsync(ma, MN * sizeof(float), CU_DEVICE_CPU, 0),
	   "Prefetch->CPU");
	ck(cuCtxSynchronize(), "sync prefetch2");
#endif
	for (int i = 0; i < MN; i++)
