...
int main(void)
{
	...
	if (r == CUDA_SUCCESS) {
		...
		printf("B GetDevicePointer      : %d %s dptr=%#llx\n", r2,
		       r2 ? es(r2) : "ok", (unsigned long long)dptr);
	...
	if (r == CUDA_SUCCESS) {
		...
		printf("C GetDevicePointer      : %d %s dptr=%#llx\n", r2,
		       r2 ? es(r2) : "ok", (unsigned long long)dreg);
	}
	...
	if (r == CUDA_SUCCESS) {
		...
		if (cuMemAlloc(&d, sz) == CUDA_SUCCESS) {
			...
			for (size_t i = 0; i < sz; i++)
				p[i] = (unsigned char)(i * 17 + 3);
			cuMemcpyHtoD(d, hbuf, sz);
			memset(hbuf, 0, sz);
			r2 = cuMemcpyDtoH(hbuf, d, sz);
			size_t bad = 0;
			for (size_t i = 0; i < sz; i++)
				if (p[i] != (unsigned char)(i * 17 + 3))
					bad++;
