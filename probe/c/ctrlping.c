...
static int reihe(int fd, uint32_t hclient, uint32_t size, long iters, int quiet)
{
	...
	for (long i = 0; i < iters; i++) {
		...
		if (r != 0 || c.status != 0) {
			if (fails == 0)
				fprintf(stderr,
					"ctrlping: size=%u ret=%d status=%#x\n",
					size, r, c.status);
			fails++;
		}
	}
	...
	if (!quiet)
		printf("ctrlping bytes=%u n=%ld fails=%ld p50_us=%.2f p90_us=%.2f p99_us=%.2f mean_us=%.2f\n",
		       ...
		       size * 3 + (unsigned)sizeof(struct build_version), iters,
		       fails, ns[(size_t)(50 * (iters - 1) / 100)] / 1000.0,
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < n_sizes; i++) {
		uint32_t size =
			argc > 2 ? (uint32_t)strtoul(argv[2 + i], NULL, 0) :
				   defaults[i];
