...
static uint32_t alloc_client(int fd)
{
	...
	int r = ioctl(fd, _IOWR(NV_IOCTL_MAGIC, NV_ESC_RM_ALLOC, struct nvos64),
		      &p);
	if (r || p.status) {
		fprintf(stderr, "alloc client ret=%d st=0x%x\n", r, p.status);
		return 0;
	}
	return p.hObjectNew;
}
...
static void query(const char *label, int fd, uint32_t self, uint32_t oclient,
		  uint32_t oobj)
{
	struct accrights ap;
	...
	ap.hObject = oobj;
	uint32_t st;
	int r = ctrl(fd, self, self, CMD_GET_ACCESS_RIGHTS, &ap, sizeof ap,
		     &st);
	printf("  %-46s ret=%d status=0x%02x mask=0x%08x  DUP=%s\n", label, r,
	       st, ap.maskResult, (ap.maskResult & 1) ? "JA" : "nein");
}

static void grant(int fd, uint32_t hclient, uint32_t hobject, uint16_t type,
		  const char *tn)
{
	struct shareobj so;
	...
	uint32_t st;
	int r = ctrl(fd, hclient, hclient, CMD_SHARE_OBJECT, &so, sizeof so,
		     &st);
...
int main(void)
{
	...
	if (pid == 0) {
		...
		query("B fragt Rechte an eigenem Client (Referenz)", fdB, cB,
		      cB, cB);
		write(ba[1], "x", 1);
		read(ab[0], &s, 1);
