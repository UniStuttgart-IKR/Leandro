...
#
# matrix-group:     egl
# matrix-libs:      libEGL_nvidia libnvidia-eglcore libnvidia-egl-wayland libnvidia-glsi libnvidia-tls libnvidia-glvkspirv
# matrix-entry:     eglGetPlatformDisplayEXT(wayland) + pbuffer + GLES2
...
out=$(lea_matrix_workload "$LEA_PROBE_BIN/eglplat" wayland 2>&1); rc=$?
echo "$out"
# Exit 2 from eglplat means it was built without this platform's headers, or
# there is no a Wayland display (WAYLAND_DISPLAY) here. That is a declared reason, not a failure.
[[ $rc -ne 2 ]] || { echo "declared-unsupported: workload not procurable in this environment -- a Wayland display (WAYLAND_DISPLAY) is not available"; exit 2; }
[[ $rc -eq 0 ]] || { error "eglplat wayland exited $rc"; exit 1; }
px=$(sed -n 's/^PIXEL=//p' <<<"$out" | tail -1)
vendor=$(sed -n 's/^EGLVENDOR=//p' <<<"$out" | tail -1)
lea_matrix_criterion "EGL/wayland: vendor $vendor, pbuffer pixel read back as $px"
