...
#
# matrix-group:     egl
# matrix-libs:      libEGL_nvidia libnvidia-eglcore libnvidia-egl-xcb libnvidia-glsi libnvidia-tls libnvidia-glvkspirv
# matrix-entry:     eglGetPlatformDisplayEXT(xcb) + pbuffer + GLES2
...
out=$(lea_matrix_workload "$LEA_PROBE_BIN/eglplat" xcb 2>&1); rc=$?
echo "$out"
...
[[ $rc -eq 0 ]] || { error "eglplat xcb exited $rc"; exit 1; }
px=$(sed -n 's/^PIXEL=//p' <<<"$out" | tail -1)
vendor=$(sed -n 's/^EGLVENDOR=//p' <<<"$out" | tail -1)
lea_matrix_criterion "EGL/xcb: vendor $vendor, pbuffer pixel read back as $px"
