...
int main(void)
{
	...
	for (i = 0; i < sizeof(measured) / sizeof(measured[0]); i++) {
		...
		printf("  balloon: %u x %llu KiB + %llu KiB\n", b.full,
		       (unsigned long long)(b.chunk >> 10),
		       (unsigned long long)(b.rest >> 10));
		CHECK("a chunk is one scanout buffer",
		      b.chunk == measured[i].scanout);
		CHECK("five of them", b.full == 5);
		CHECK("the rest holds four cursors",
		      b.rest >= NVRM_RESERVE_CURSOR_BYTES);
		CHECK("the rest is less than a buffer", b.rest < b.chunk);
		CHECK("the chunks add up to R",
		      b.full * b.chunk + b.rest == (u64)mib << 20);
	}

	/* A fixed R too large for chunks of S: at most the cap, still R. */
	nvrm_balloon_shape(1024ull << 20, 0x870000, &b);
	CHECK("a large R is cut into at most the cap",
	      b.full + (b.rest != 0) <= NVRM_BALLOON_MAX_CHUNKS);
	CHECK("a large R still adds up",
	      b.full * b.chunk + b.rest == 1024ull << 20);
	CHECK("a large R's chunk is whole 64 KiB", !(b.chunk & 0xffff));
	nvrm_balloon_shape(4ull << 20, 0x870000, &b);
	CHECK("an R below one buffer is all rest",
	      b.full == 0 && b.rest == 4ull << 20);
	nvrm_balloon_shape(44ull << 20, 0, &b);
	CHECK("no display size still cuts R",
	      b.full * b.chunk + b.rest == 44ull << 20 && b.chunk);
	...
	CHECK("a cursor takes the rest",
	      nvrm_balloon_pick(5, 1, b.rest, 0x40000) == 0);
	CHECK("a scanout buffer takes a whole chunk",
	      nvrm_balloon_pick(5, 1, b.rest, 0x870000) == 1);
	CHECK("a cursor takes a whole chunk once the rest is gone",
	      nvrm_balloon_pick(5, 0, b.rest, 0x40000) == 1);
	CHECK("a scanout buffer takes the rest last",
	      nvrm_balloon_pick(0, 1, b.rest, 0x870000) == 0);
	CHECK("an empty balloon gives nothing",
	      nvrm_balloon_pick(0, 0, b.rest, 0x40000) == -1);
	...
	CHECK("the refused scanout buffer",
	      nvrm_balloon_eligible(NVRM_CLASS_MEMORY_LOCAL_USER, a));
	...
	CHECK("NO_SCANOUT is not a display buffer",
	      !nvrm_balloon_eligible(NVRM_CLASS_MEMORY_LOCAL_USER, a));
	ask(a, 0x1102, 0x10020000, 0x40008);
	CHECK("NO_SCANOUT with ISO is still not",
	      !nvrm_balloon_eligible(NVRM_CLASS_MEMORY_LOCAL_USER, a));
	ask(a, 0x102, 0x10020000, 0x8);
	CHECK("without ISO it is not",
	      !nvrm_balloon_eligible(NVRM_CLASS_MEMORY_LOCAL_USER, a));
	ask(a, 0x102, 0x10020000 | (1u << 25), 0x40008);
	CHECK("system memory is not",
	      !nvrm_balloon_eligible(NVRM_CLASS_MEMORY_LOCAL_USER, a));
	ask(a, 0x102 | NVRM_NVOS32_ALLOC_FLAGS_VIRTUAL, 0x10020000, 0x40008);
	CHECK("a virtual range is not",
	      !nvrm_balloon_eligible(NVRM_CLASS_MEMORY_LOCAL_USER, a));
	ask(a, 0x102, 0x10020000, 0x40008);
	...
	CHECK("a chunk is a display buffer",
	      nvrm_balloon_eligible(NVRM_CLASS_MEMORY_LOCAL_USER, a));
	CHECK("a chunk has the refused buffer's attr, in pitch format",
	      nvrm_vram_rd32(a, NVRM_MEMALLOC_ATTR_OFF) == 0x10000000);
	CHECK("a chunk has NVKMS's flags",
	      nvrm_vram_rd32(a, NVRM_MEMALLOC_FLAGS_OFF) == 0x102);
	CHECK("a chunk has NVKMS's attr2",
	      nvrm_vram_rd32(a, NVRM_MEMALLOC_ATTR2_OFF) == 0x40008);
	CHECK("a chunk is PRIMARY",
	      nvrm_vram_rd32(a, NVRM_MEMALLOC_TYPE_OFF) == 8);
	...
	CHECK("no display, no reserve",
	      nvrm_display_reserve_auto_mib(0, 1080) == 0);
	CHECK("an absurd size is not a display",
	      nvrm_display_reserve_auto_mib(1 << 30, 1 << 30) == 0);
	CHECK("the reserve grows with the display",
	      nvrm_display_reserve_auto_mib(2560, 1440) >
		      nvrm_display_reserve_auto_mib(1920, 1080));
