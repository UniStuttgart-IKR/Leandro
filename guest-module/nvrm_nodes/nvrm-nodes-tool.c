...
static int open_dev(void)
{
	...
	if (fd < 0) {
		perror(LEA_DEV_PATH);
		fprintf(stderr,
			"  module loaded? (sudo insmod nvrm_nodes.ko)\n");
		exit(1);
	}
	return fd;
}
...
static int cmd_gpa(size_t mib, unsigned hold_s)
{
	...
	if (psl <= 0) {
		fprintf(stderr,
			"nvrm-nodes-tool: sysconf(_SC_PAGESIZE) gave %ld\n",
			psl);
		return 1;
	}
	ps = (size_t)psl;
	...
	if (posix_memalign((void **)&buf, ps, len)) {
	...
	printf("nvrm-nodes-tool: %zu MiB -> %u runs, sum %llu bytes %s\n", mib,
	       req.runs_len, (unsigned long long)total,
	       total == len ? "ok" : "WRONG");
	...
	printf("nvrm-nodes-tool: runs_max=0 -> rc=%d needed=%u %s\n", rc,
	       req.runs_len, (rc < 0 && req.runs_len > 0) ? "ok" : "WRONG");
...
int main(int argc, char **argv)
{
	...
	if ((argc == 3 || argc == 4) && !strcmp(argv[1], "gpa"))
		return cmd_gpa(strtoul(argv[2], NULL, 0),
			       argc == 4 ? (unsigned)strtoul(argv[3], NULL, 0) :
					   0);
